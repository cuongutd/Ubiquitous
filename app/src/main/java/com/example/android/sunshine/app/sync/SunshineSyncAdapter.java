...
public class SunshineSyncAdapter extends AbstractThreadedSyncAdapter implements DataApi.DataListener, GoogleApiClient.ConnectionCallbacks,
    ...
    //these are for sending data to watch
    ...
    private Asset createAssetFromBitmap(int weatherId){
        ...
        Drawable backgroundDrawable = resources.getDrawable(Utility.getArtResourceForWeatherCondition(weatherId));
    ...
    private void updateWatchFace() {
        Log.d(LOG_TAG, "updateWatchFace");
        getTodayWeather();
        PutDataMapRequest putDataMapRequest = PutDataMapRequest.create(SUNSHINE_DATA_PATH);
        putDataMapRequest.getDataMap().putString(SUNSHINE_DATA_HIGH, Utility.formatTemperature(getContext(), mTodayHigh));
        putDataMapRequest.getDataMap().putString(SUNSHINE_DATA_LOW, Utility.formatTemperature(getContext(), mTodayLow));
        Asset asset = createAssetFromBitmap(mTodayWeatherId);
        putDataMapRequest.getDataMap().putAsset(SUNSHINE_DATA_IMG, asset);
...
//        }
    ...
    private void getTodayWeather(){
        ...
        if (cursor.moveToFirst()) {
            mTodayWeatherId = cursor.getInt(INDEX_WEATHER_ID);
            mTodayHigh = cursor.getDouble(INDEX_MAX_TEMP);
            mTodayLow = cursor.getDouble(INDEX_MIN_TEMP);
            mTodayDesc = cursor.getString(INDEX_SHORT_DESC);
        }
